...
class DoubleGate extends StatefulWidget {
  ...
  const DoubleGate({
    ...
    this.width = 320.0,
    this.height = 160.0,
    this.frameRadius = 4.0, // Ширина нашего зазора по центру
  ...
  @override
  State<DoubleGate> createState() => _DoubleGateState();
}

class _DoubleGateState extends State<DoubleGate>
  ...
  @override
  void didUpdateWidget(covariant DoubleGate oldWidget) {
  ...
  void _handleInput(Offset localPosition, {bool isDrag = false}) {
    ...
    // Считаем максимальную ширину активной части с учетом зазора
    double gap = widget.frameRadius;
    double maxActiveWidth = (widget.width - gap) / 2;
    ...
    if (isDrag &&
      ...
      if (_isLeftHalf!) {
        // За левую штору
        delta = (localPosition.dx - _dragStartPos!) / maxActiveWidth;
      } else {
        // За правую штору
        delta = (_dragStartPos! - localPosition.dx) / maxActiveWidth;
      }
      newNormalized = (_dragStartValue! + delta).clamp(0.0, 1.0);
    } else {
      // Прямой тап
      bool isLeft = localPosition.dx < (widget.width / 2);
      if (isLeft) {
        newNormalized = (localPosition.dx / maxActiveWidth).clamp(0.0, 1.0);
      } else {
        newNormalized = ((widget.width - localPosition.dx) / maxActiveWidth)
            .clamp(0.0, 1.0);
      }
    }
...
class _DoubleCurtainPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(leftActiveRRect, Paint()..color = active);
  ...
  @override
  bool shouldRepaint(covariant _DoubleCurtainPainter old) => true;
}
