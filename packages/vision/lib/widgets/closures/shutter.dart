...
class _ShutterState extends State<Shutter> with SingleTickerProviderStateMixin {
  ...
  void _handleInput(Offset localPosition, {bool isDrag = false}) {
    ...
    if (isDrag &&
        ...
        _dragStartPos != null) {
      double delta = (localPosition.dy - _dragStartPos!) / widget.height;
      newNormalized = (_dragStartValue! + delta).clamp(0.0, 1.0);
    } else {
      newNormalized = (localPosition.dy / widget.height).clamp(0.0, 1.0);
    }
      ...
      child: AnimatedBuilder(
        ...
        builder: (c, _) => TweenAnimationBuilder<double>(
          ...
          builder: (c, val, _) => CustomPaint(
            ...
            painter: _VerticalCurtainPainter(
              val,
...
class _VerticalCurtainPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final activeRRect = RRect.fromRectAndCorners(
      Rect.fromLTWH(0, 0, size.width, activeHeight),
      topLeft: Radius.circular(frameRadius),
      topRight: Radius.circular(frameRadius),
      bottomLeft: Radius.circular(edgeRadius),
      bottomRight: Radius.circular(edgeRadius),
  ...
  @override
  bool shouldRepaint(covariant _VerticalCurtainPainter old) => true;
}
